...
/********************* lekirsch@lbl.gov ***************************************/
// this version was updated using AI to remove compiler warnings; biggest change is switching from 
//variable-length array (VLA) to std::vector<double>
//so here AI magic did its work - use at your own risk!! but changes were small and 
//this version should be safe to use imho
// Dennis Muecher - November 2025 - muecher@ikp.uni-koeln.de	
...
bool TakeStep(int &nConEx, int &nSpb, int &nPar, int &nDisEx, int &nLvlInBin,
  ...
  (void)nReal; // suppress unused parameter warning
...
void GetExI(int &nExI, int &nSpbI, int &nParI, int &nDisEx, int &nLvlInBinI,
  ...
  (void)dExIMean; (void)dExISpread; // suppress unused parameter warnings
...
void InitFn() {
  ...
  int nJIntSize = static_cast<int>(g_dPlotSpMax+0.6+1);
  std::vector<double> adJInt(nJIntSize);
 // densities for the underlying J
